...
### Boundaries
**User Input to Pipeline (Direct)**: The user provides raw text input. *Failure mode:* The query could be completely empty, massive, or contain harmful characters. This boundary is protected by Pydantic schema validation which enforces a 3 to 900 character limit.
...
- **Retrieval Evaluation:** The retrieval step returns a maximum of three docs per search. Therefore retrieval can be evaluated using the metrics *Precision@3* and *nDCG@3*. *Precision@3* measures how many of the top 3 retrieved docs belong to the expected source docs. *nDCG@3* is similar, but also evaluates the order of those retrieved docs. We will track these metrics over a unit test dataset with a target threshold of **0.80 for Precision@3** and **0.85 for nDCG@3**.
...
- **Security:** Web-based HTML and Markdown stripping has been removed. Instead, the database builder relies on a `has_exotic_characters` check and `strip_exotic_characters` logic to clean local text files before ingestion, ensuring clean tokenization for the embedding model and removing potential injection vectors.
## 4.4 Reliability and Security Decisions
- **Retry strategy:** Network and API connection failures to the local LLM microservices (ports `8000`-`8003`) are retriable. The system uses the `tenacity` library with an exponential backoff scheme (waiting between 2 and 10 seconds), up to a maximum of 3 attempts, before gracefully catching the exception.
- **Circuit breakers:** The retrieval dependency (ChromaDB) has a semantic circuit breaker. It enforces a rigid relevance cutoff (`MIN_SIMILARITY_THRESHOLD` of `0.5`). If retrieved documents fail to exceed this threshold, the pipeline trips the circuit breaker, bypassing LLM generation entirely to prevent hallucination.
- **Idempotency:** The pipeline contains absolutely no write operations during execution. All tool calls and inference requests are strictly read-only. Consequently, duplicate executions caused by user retries are inherently safe and cannot corrupt system state.
- **Trust boundaries:** Externally retrieved content (from local documents) is strictly isolated from system instructions via structural formatting. The prompt template clearly demarcates `System Context:
[Content]` separate from `User Question:
[Query]`, and the `MAX_PROMPT_WORDS` (2000) truncation ensures that maliciously large documents cannot overflow the context window and override instructions.
- **Memory security:** Persistent memory only exists in the ChromaDB vector store. Write-time validation is implemented during document ingestion in `database_builder.py`, where `has_exotic_characters` and `clean_text` strictly sanitize the text, rejecting or stripping dangerous characters before vectorization.
## 4.5 Deployment Plan
- **Container strategy:** For production, this application requires an orchestrated container approach, such as `docker-compose`. The central pipeline is packaged as one container (requiring Python, Pydantic, and ChromaDB), while the local model server (`model_server.py`) is packaged in four distinct containers (requiring `uvicorn` and HuggingFace dependencies) mapped to ports `8000`-`8003`.
- **Secrets management:** As the system relies entirely on local models and a local ChromaDB, it currently requires no external API keys. If external APIs were introduced, credentials would be stored in AWS Secrets Manager and injected as environment variables at runtime.
- **State management:** The pipeline execution is completely stateless in-memory. The only state that lives outside the container is the vectorized document knowledge, which is stored in the persistent ChromaDB directory (`./my_chroma_db`) mapped to a persistent volume.
- **Edge considerations:** The multi-model approach makes edge deployment difficult. While the embedding model and a single LLM (like Qwen) could run on a local device CPU, simultaneously hosting Qwen, Llama, Mistral, and Phi requires significant VRAM and compute. Therefore, no component benefits from edge deployment; all tasks must run on a centralized, GPU-backed server.
